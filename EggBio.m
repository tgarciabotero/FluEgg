function [D,Rhoe_ref]=EggBio(time,specie)
...
if strcmp(specie,'Silver')%if specie=='Silver'
    ...
    a=20.5290;b=2262.9;c=999.05;Rhoe_ref=(a*exp(-time/b))+c;%R2 = 0.82 for silver carp eggs
else
    Dmin=1.5970;% mm
    ...
    %% Density of eggs fit Standardized to 22C
    a=24.928;b=1730.1;c=999.08;Rhoe_ref=(a*exp(-time/b))+c;%R2 = 0.82 for silver carp eggs 
end
...
Rhoevar(t,1)=normrnd(Rhoe_ref(t),RhoeStd);
while (Rhoevar(t)>=Rhoe_ref(t)+RhoeStd)||(Rhoevar(t)<=Rhoe_ref(t)-RhoeStd)
    Rhoevar(t)=normrnd(Rhoe_ref(t),RhoeStd);
end
end
Rhoe_ref=Rhoevar;
D=Dvar;
